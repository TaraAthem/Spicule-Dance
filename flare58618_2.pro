...
;CD, '/Users/dmitriikolotkov/Dropbox/Tishtrya/flare58618'
 
A = FindGen(16) * (!PI*2/16.) & UserSym, cos(A), sin(A), /fill
device, decomposed=0 & loadct,39
d=get_screen_size()
flarename= 'flare58618.fits'
...
;for SF =0.001, 0.3001, 0.005 DO BEGIN   
;Print, 'Shift-Factor=', SF
set_plot, 'PS'
;device, filename='Flare58618_PREFLARE_2' + StrTrim(SF,2)+'.eps', encapsulated=1,$		;DEFINES FILENAME AND IMAGE PROPERTIES
device, filename='58618_1.eps', encapsulated=1,$
...
modes=emd(x_pre-mean(x_pre), shiftfactor=0.05, epsilon=1d-4, maxsiftings=1d5)
...
modes=emd(x_flare-mean(x_flare), shiftfactor=0.111, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
;0.03
modes=emd(x_flare-mean(x_flare), shiftfactor=0.03, epsilon=1d-4, maxsiftings=1d5)
...
;SF=
modes=emd(x_post-mean(x_post), shiftfactor=0.226, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
plot,EXP(-freq_flare), pow_flare,title='Fourier Spectrum - Flare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-4.,3.3]
...
plot,EXP(-freq_post), pow_post,title='Fourier Spectrum - Postflare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], yrange=[-3., 2.7], thick=2
...
plot,[0],[0],charsize=ch,/xsty,/ysty,yrange=[-12,25],xrange=[0,300],$   ;ORIGINAL SIGNAL WINDOW
  title='Profile of Flare 58618', xtitle='Elapsed Time [s]',ytitle='Amplitude'
