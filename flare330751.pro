...
;CD, '/Users/dmitriikolotkov/Dropbox/Tishtrya/flare330751'
 
A = FindGen(16) * (!PI*2/16.) & UserSym, cos(A), sin(A), /fill
device, decomposed=0 & loadct,39
d=get_screen_size()
flarename= 'flare330751.fits'
...
;for SF =0.001, 0.300, 0.002 DO BEGIN   
;Print, 'Shift-Factor=', SF
set_plot, 'PS'
;device, filename='Flare330751_TREND_' + StrTrim(SF,2)+'.eps', encapsulated=1,$		;DEFINES FILENAME AND IMAGE PROPERTIES
device, filename='330751_1.eps', encapsulated=1,$
...
;Real sf= 
;0.151
modes=emd(x_pre-mean(x_pre), shiftfactor=0.001, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
trend_emd_pre=mean(x_pre)+modes[*,sz[2]-1] ; +modes[*, sz[2]-2] ;+modes[*, sz[2]-4]   ;FIND TREND FROM LAST MODE IN EMD SPECTRUM
...
modes=emd(x_flare-mean(x_flare), shiftfactor=0.161, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
;0.001
modes=emd(x_flare-mean(x_flare), shiftfactor=0.072, epsilon=1d-4, maxsiftings=1d5)
...
modes=emd(x_post-mean(x_post), shiftfactor=0.065, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
;SF= 0.58 for mode[1]
modes=emd(x_post-mean(x_post), shiftfactor=0.001, epsilon=1d-4, maxsiftings=1d5)
...
plot,EXP(-freq_pre), pow_pre,title='Fourier Spectrum - Preflare', xtitle=$   ;FOURIER POWER SPECTRUM
  ...
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-3.,2.8]
...
plot,EXP(-freq_flare), pow_flare,title='Fourier Spectrum - Flare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-3.2,4.2]
...
plot,EXP(-freq_post), pow_post,title='Fourier Spectrum - Postflare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], yrange=[-4.7, 3.1], thick=2
...
plot,[0],[0],charsize=ch,/xsty,/ysty,yrange=[-20.,26.],xrange=[0,300],$   ;ORIGINAL SIGNAL WINDOW
  title='Profile of Flare 330751', xtitle='Elapsed Time [s]',ytitle='Amplitude'
oplot,tim_flare,trend_emd_flare  -35. ,color=130,thick=4
oplot,tim_flare, x_flare+trend_emd_flare -35.,thick=3

oplot,tim_flare,x_flare
oplot,tim_flare,emd_mode_flare,color=250,thick=4

oplot,tim_flare,x_flare-emd_mode_flare -13. 

oplot,tim_pre, x_pre+trend_emd_pre -35.
oplot,tim_pre,trend_emd_pre -35. ,color=50,thick=4	 ;200 is yelow, 150 is lime green, 250 red, 50 lbue

oplot,tim_pre,x_pre
;oplot,tim_pre,emd_mode_pre,color=130,thick=4

oplot,tim_pre,x_pre -13. 

oplot,tim_post, x_post+trend_emd_post -35.,thick=3
oplot,tim_post,trend_emd_post -35. ,color=50,thick=4

oplot,tim_post,x_post
oplot,tim_post,emd_mode_post,color=150,thick=4

oplot,tim_post,x_post -13. ;-emd_mode_post -7. 
