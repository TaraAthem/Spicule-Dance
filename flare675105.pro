...
;CD, '/Users/dmitriikolotkov/Dropbox/Tishtrya/flare675105'
 
A = FindGen(16) * (!PI*2/16.) & UserSym, cos(A), sin(A), /fill
device, decomposed=0 & loadct,39
d=get_screen_size()
flarename= 'flare675105.fits'
...
set_plot, 'PS'
;device, filename='Flare675105_EMD_' + StrTrim(SF,2)+'.eps', encapsulated=1,$		;DEFINES FILENAME AND IMAGE PROPERTIES
device, filename='675105_1.eps', encapsulated=1,$
...
modes=emd(x_pre-mean(x_pre), shiftfactor=0.3, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
modes=emd(x_flare-mean(x_flare), shiftfactor=0.0165, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
;0.25
modes=emd(x_flare-mean(x_flare), shiftfactor=0.001, epsilon=1d-4, maxsiftings=1d5)
...
;SF=
modes=emd(x_post-mean(x_post), shiftfactor=0.3, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
;SF= 0.0002 pretty good 
modes=emd(x_post-mean(x_post), shiftfactor=0.58, epsilon=1d-4, maxsiftings=1d5)
...
plot,EXP(-freq_pre), pow_pre,title='Fourier Spectrum - Preflare', xtitle=$   ;FOURIER POWER SPECTRUM
  ...
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-3.9,2.6]
...
plot,EXP(-freq_flare), pow_flare,title='Fourier Spectrum - Flare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-5.3,2.9]
...
plot,EXP(-freq_post), pow_post,title='Fourier Spectrum - Postflare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], yrange=[-4.8, 3.1], thick=2
...
plot,[0],[0],charsize=ch,/xsty,/ysty,yrange=[-20.,23.],xrange=[0,300],$   ;ORIGINAL SIGNAL WINDOW
  title='Profile of Flare 675105', xtitle='Elapsed Time [s]',ytitle='Amplitude'
oplot,tim_flare,trend_emd_flare  -15. ,color=130,thick=4
oplot,tim_flare, x_flare+trend_emd_flare -15.,thick=3
...
oplot,tim_pre, x_pre+trend_emd_pre -15.
oplot,tim_pre,trend_emd_pre -15. ,color=50,thick=4	 ;200 is yelow, 150 is lime green, 250 red, 50 lbue
...
oplot,tim_post, x_post+trend_emd_post -15.,thick=3
oplot,tim_post,trend_emd_post -15. ,color=50,thick=4
