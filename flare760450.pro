...
;CD, '/Users/dmitriikolotkov/Dropbox/Tishtrya/flare760450'
 
A = FindGen(16) * (!PI*2/16.) & UserSym, cos(A), sin(A), /fill
device, decomposed=0 & loadct,39
d=get_screen_size()
flarename= 'flare760450.fits'
...
;for SF =0.001, 0.300, 0.05 DO BEGIN   
;Print, 'Shift-Factor=', SF
set_plot, 'PS'
;device, filename='Flare760450_EMD2_' + StrTrim(SF,2)+'.eps', encapsulated=1,$		;DEFINES FILENAME AND IMAGE PROPERTIES
device, filename='760450_1.eps', encapsulated=1,$
...
;Real sf= 
;0.151
modes=emd(x_pre-mean(x_pre), shiftfactor=0.0001, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
;near .204
modes=emd(x_pre-mean(x_pre), shiftfactor=0.0027, epsilon=1d-4, maxsiftings=1d5)
...
;SF= 
;around 0.230
modes=emd(x_flare-mean(x_flare), shiftfactor=0.1050, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
;0.205
modes=emd(x_post-mean(x_post), shiftfactor=0.187, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
plot,EXP(-freq_pre), pow_pre,title='Fourier Spectrum - Preflare', xtitle=$   ;FOURIER POWER SPECTRUM
  ...
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-2.7,2.2]
...
plot,EXP(-freq_flare), pow_flare,title='Fourier Spectrum - Flare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-5.,3.]
...
plot,EXP(-freq_post), pow_post,title='Fourier Spectrum - Postflare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], yrange=[-4.1, 2.5], thick=2
...
plot,[0],[0],charsize=ch,/xsty,/ysty,yrange=[-22.,35.],xrange=[0,300],$   ;ORIGINAL SIGNAL WINDOW
  title='Profile of Flare 760450', xtitle='Elapsed Time [s]',ytitle='Amplitude'
oplot,tim_flare,trend_emd_flare - 20. ,color=130,thick=4
oplot,tim_flare, x_flare+trend_emd_flare- 20.,thick=3

oplot,tim_flare,x_flare
oplot,tim_flare,emd_mode_flare,color=250,thick=4

oplot,tim_flare,x_flare-emd_mode_flare -15. 

oplot,tim_pre, x_pre+trend_emd_pre - 20.
oplot,tim_pre,trend_emd_pre - 20. ,color=50,thick=4	 ;200 is yelow, 150 is lime green, 250 red, 50 lbue

oplot,tim_pre,x_pre
oplot,tim_pre,emd_mode_pre,color=130,thick=4

oplot,tim_pre,x_pre-15.   

oplot,tim_post, x_post+trend_emd_post - 20.,thick=3
oplot,tim_post,trend_emd_post -20. ,color=50,thick=4

oplot,tim_post,x_post
;oplot,tim_post,emd_mode_post,color=150,thick=4

oplot,tim_post,x_post-15.   ;-emd_mode_post -5. 
