...
;CD, '/Users/dmitriikolotkov/Dropbox/Tishtrya/flare724818'
 
A = FindGen(16) * (!PI*2/16.) & UserSym, cos(A), sin(A), /fill
device, decomposed=0 & loadct,39
d=get_screen_size()
flarename= 'flare724818.fits'
...
;for SF =0.001, 0.300, 0.009 DO BEGIN   
;Print, 'Shift-Factor=', SF
set_plot, 'PS'
;device, filename='Flare724818_EMD2_' + StrTrim(SF,2)+'.eps', encapsulated=1,$		;DEFINES FILENAME AND IMAGE PROPERTIES
device, filename='724818_1.eps', encapsulated=1,$
...
modes=emd(x_pre-mean(x_pre), shiftfactor=0.106, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
;if it exists- between 0.109-0.127
modes=emd(x_pre-mean(x_pre), shiftfactor=0.117, epsilon=1d-4, maxsiftings=1d5)
...
modes=emd(x_flare-mean(x_flare), shiftfactor=0.295, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
;0.212
modes=emd(x_flare-mean(x_flare), shiftfactor=0.109, epsilon=1d-4, maxsiftings=1d5)
...
;SF=0.016
modes=emd(x_post-mean(x_post), shiftfactor=0.0155, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
;SF= 0.235 pretty good 
;0.182 gives other mode
modes=emd(x_post-mean(x_post), shiftfactor=0.1805, epsilon=1d-4, maxsiftings=1d5)
...
plot,EXP(-freq_pre), pow_pre,title='Fourier Spectrum - Preflare', xtitle=$   ;FOURIER POWER SPECTRUM
  ...
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-4.,2.]
...
plot,EXP(-freq_flare), pow_flare,title='Fourier Spectrum - Flare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-5.3,3.5]
...
plot,EXP(-freq_post), pow_post,title='Fourier Spectrum - Postflare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], yrange=[-4., 2.], thick=2
...
plot,[0],[0],charsize=ch,/xsty,/ysty,yrange=[-20.,23.],xrange=[0,300],$   ;ORIGINAL SIGNAL WINDOW
  title='Profile of Flare 724818', xtitle='Elapsed Time [s]',ytitle='Amplitude'
oplot,tim_flare,trend_emd_flare - 14. ,color=130,thick=4
oplot,tim_flare, x_flare+trend_emd_flare -14.,thick=3

oplot,tim_flare,x_flare
oplot,tim_flare,emd_mode_flare,color=250,thick=4

oplot,tim_flare,x_flare-emd_mode_flare -10. 

oplot,tim_pre, x_pre+trend_emd_pre - 14.
oplot,tim_pre,trend_emd_pre -14. ,color=50,thick=4	 ;200 is yelow, 150 is lime green, 250 red, 50 lbue

oplot,tim_pre,x_pre
;oplot,tim_pre,emd_mode_pre,color=130,thick=4

oplot,tim_pre,x_pre -10. 

oplot,tim_post, x_post+trend_emd_post - 14.,thick=3
oplot,tim_post,trend_emd_post -14. ,color=50,thick=4

oplot,tim_post,x_post
oplot,tim_post,emd_mode_post,color=150,thick=4

oplot,tim_post,x_post -10. ;-emd_mode_post -7. 
