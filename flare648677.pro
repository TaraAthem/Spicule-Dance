...
;CD, '/Users/dmitriikolotkov/Dropbox/Tishtrya/flare648677'
 
A = FindGen(16) * (!PI*2/16.) & UserSym, cos(A), sin(A), /fill
device, decomposed=0 & loadct,39
d=get_screen_size()
flarename= 'flare648677.fits'
...
;for SF =0.01, 0.100, 0.005 DO BEGIN   
;Print, 'Shift-Factor=', SF
set_plot, 'PS'
;device, filename='Flare648677_EMD_' + StrTrim(SF,2)+'.eps', encapsulated=1,$		;DEFINES FILENAME AND IMAGE PROPERTIES
device, filename='648677_1.eps', encapsulated=1,$
...
modes=emd(x_pre-mean(x_pre), shiftfactor=0.09, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
modes=emd(x_flare-mean(x_flare), shiftfactor=0.175, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
emd_mode_flare=  modes[*,3];+modes[*,2]
...
modes=emd(x_post-mean(x_post), shiftfactor=0.22, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
;SF=0.019 almost CI
modes=emd(x_post-mean(x_post), shiftfactor=0.015, epsilon=1d-4, maxsiftings=1d5)
...
plot,EXP(-freq_pre), pow_pre,title='Fourier Spectrum - Preflare', xtitle=$   ;FOURIER POWER SPECTRUM
  ...
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-3.5,3.1]
...
plot,EXP(-freq_post), pow_post,title='Fourier Spectrum - Postflare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], yrange=[-4.7, 3.5], thick=2
...
plot,[0],[0],charsize=ch,/xsty,/ysty,yrange=[-15,27],xrange=[0,300],$   ;ORIGINAL SIGNAL WINDOW
  title='Profile of Flare 648677', xtitle='Elapsed Time [s]',ytitle='Amplitude'
oplot,tim_flare,trend_emd_flare  -40. ,color=130,thick=4
oplot,tim_flare, x_flare+trend_emd_flare -40.,thick=3
...
oplot,tim_pre, x_pre+trend_emd_pre -40.
oplot,tim_pre,trend_emd_pre -40. ,color=50,thick=4	 ;200 is yelow, 150 is lime green, 250 red, 50 lbue
...
oplot,tim_post, x_post+trend_emd_post -40.,thick=3
oplot,tim_post,trend_emd_post -40. ,color=50,thick=4
