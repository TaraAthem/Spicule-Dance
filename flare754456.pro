...
;CD, '/Users/dmitriikolotkov/Dropbox/Tishtrya/flare754456'
 
A = FindGen(16) * (!PI*2/16.) & UserSym, cos(A), sin(A), /fill
device, decomposed=0 & loadct,39
d=get_screen_size()
flarename= 'flare754456.fits'
...
set_plot, 'PS'
;device, filename='Flare754456_TREND_' + StrTrim(SF,2)+'.eps', encapsulated=1,$		;DEFINES FILENAME AND IMAGE PROPERTIES
device, filename='754456_1.eps', encapsulated=1,$
...
modes=emd(x_pre-mean(x_pre), shiftfactor=0.013, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
modes=emd(x_pre-mean(x_pre), shiftfactor=0.03, epsilon=1d-4, maxsiftings=1d5)
...
modes=emd(x_flare-mean(x_flare), shiftfactor=0.025, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
;0.001
modes=emd(x_flare-mean(x_flare), shiftfactor=0.002, epsilon=1d-4, maxsiftings=1d5)
...
modes=emd(x_post-mean(x_post), shiftfactor=0.013, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
trend_emd_post=mean(x_post)+modes[*,sz[2]-1] +modes[*, sz[2]-3] ;+modes[*, sz[2]-2]    ;FIND TREND FROM LAST MODE IN EMD SPECTRUM
...
plot,EXP(-freq_flare), pow_flare,title='Fourier Spectrum - Flare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-5.,2.6]
...
plot,EXP(-freq_post), pow_post,title='Fourier Spectrum - Postflare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], yrange=[-4.7, 4.5], thick=2
...
plot,[0],[0],charsize=ch,/xsty,/ysty,yrange=[-15,27],xrange=[0,300],$   ;ORIGINAL SIGNAL WINDOW
  title='Profile of Flare 754456', xtitle='Elapsed Time [s]',ytitle='Amplitude'
oplot,tim_flare,trend_emd_flare  -87. ,color=130,thick=4
oplot,tim_flare, x_flare+trend_emd_flare -87.,thick=3
...
oplot,tim_pre, x_pre+trend_emd_pre -87.
;oplot,tim_pre,trend_emd_pre -87. ,color=50,thick=4	 ;200 is yelow, 150 is lime green, 250 red, 50 lbue
...
oplot,tim_post, x_post+trend_emd_post -87.,thick=3
oplot,tim_post,trend_emd_post -87. ,color=50,thick=4
