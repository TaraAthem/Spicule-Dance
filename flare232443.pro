...
;CD, '/Users/dmitriikolotkov/Dropbox/Tishtrya/flare232443'
 
A = FindGen(16) * (!PI*2/16.) & UserSym, cos(A), sin(A), /fill
device, decomposed=0 & loadct,39
d=get_screen_size()
flarename= 'flare232443.fits'
...
set_plot, 'PS'
device, filename='232443_1.eps', encapsulated=1,$		;DEFINES FILENAME AND IMAGE PROPERTIES
...
modes=emd(x_pre-mean(x_pre), shiftfactor=0.007, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
modes=emd(x_pre-mean(x_pre), shiftfactor=0.035, epsilon=1d-4, maxsiftings=1d5)
...
confn1_95_pre=emd_noise_conf(N_pre, alpha_1, nsamples=100l, confidence_level = 0.05d)
confn1_99_pre=emd_noise_conf(N_pre, alpha_1, nsamples=100l, confidence_level = 0.01d)
...
confn2_95_pre=emd_noise_conf(N_pre, alpha_2, nsamples=100l, confidence_level = 0.05d)
confn2_99_pre=emd_noise_conf(N_pre, alpha_2, nsamples=100l, confidence_level = 0.01d)
...
confn1_95_flare=emd_noise_conf(N_flare, alpha_1, nsamples=100l, confidence_level = 0.05d)
confn1_99_flare=emd_noise_conf(N_flare, alpha_1, nsamples=100l, confidence_level = 0.01d)
...
confn2_95_flare=emd_noise_conf(N_flare, alpha_2, nsamples=100l, confidence_level = 0.05d)
confn2_99_flare=emd_noise_conf(N_flare, alpha_2, nsamples=100l, confidence_level = 0.01d)
...
modes=emd(x_post-mean(x_post), shiftfactor=0.13, epsilon=1d-4, maxsiftings=1d5)
...
confn1_95_post=emd_noise_conf(N_post, alpha_1, nsamples=100l, confidence_level = 0.05d)
confn1_99_post=emd_noise_conf(N_post, alpha_1, nsamples=100l, confidence_level = 0.01d)
...
confn2_95_post=emd_noise_conf(N_post, alpha_2, nsamples=100l, confidence_level = 0.05d)
confn2_99_post=emd_noise_conf(N_post, alpha_2, nsamples=100l, confidence_level = 0.01d)
...
plot,EXP(-freq_pre), pow_pre,title='Fourier Spectrum - Preflare', xtitle=$   ;FOURIER POWER SPECTRUM
  ...
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-4.5,2.7]
...
plot,EXP(-freq_flare), pow_flare,title='Fourier Spectrum - Flare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-6.5,3.5]
...
plot,EXP(-freq_post), pow_post,title='Fourier Spectrum - Postflare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], yrange=[-5.5, 2.7], thick=2
...
plot,[0],[0],charsize=ch,/xsty,/ysty,yrange=[-10,20],xrange=[0,300],$   ;ORIGINAL SIGNAL WINDOW
  title='Profile of Flare 232443', xtitle='Elapsed Time [s]',ytitle='Amplitude'
oplot,tim_flare,trend_emd_flare +3. ,color=130,thick=4
oplot,tim_flare, x_flare+trend_emd_flare +3.,thick=3
...
oplot,tim_pre, x_pre+trend_emd_pre + 3.
oplot,tim_pre,trend_emd_pre +3. ,color=50,thick=4	 ;200 is yelow, 150 is lime green, 250 red, 50 lbue
...
oplot,tim_post, x_post+trend_emd_post + 3.,thick=3
oplot,tim_post,trend_emd_post +3. ,color=50,thick=4
