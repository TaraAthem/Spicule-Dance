...
;CD, '/Users/dmitriikolotkov/Dropbox/Tishtrya/flare375344'
 
A = FindGen(16) * (!PI*2/16.) & UserSym, cos(A), sin(A), /fill
device, decomposed=0 & loadct,39
d=get_screen_size()
flarename= 'flare375344.fits'
...
set_plot, 'PS'
device, filename='375344_1.eps', encapsulated=1,$		;DEFINES FILENAME AND IMAGE PROPERTIES
...
modes=emd(x_pre-mean(x_pre), shiftfactor=0.27, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
modes=emd(x_flare-mean(x_flare), shiftfactor=0.096, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
modes=emd(x_flare-mean(x_flare), shiftfactor=0.032, epsilon=1d-4, maxsiftings=1d5)
...
modes=emd(x_post-mean(x_post), shiftfactor=0.0047, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
modes=emd(x_post-mean(x_post), shiftfactor=0.092, epsilon=1d-4, maxsiftings=1d5)
...
ind_fit_1=where(nmode ne sz[2] and period_post lt break_point) ;ALLOWS A COMPONANT 1.1OF NOISE TO BE NEGLECTED IF THERE ARE INSUFFIENT
if (n_elements(ind_fit_1) lt 2) then begin      ;MODES IN THIS REGION TO ALLOW A FITTING TO BE CARRIED OUT
  print,'PORTION OF THE NOISE WITH ALPHA = ',$
    string(alpha_1,format='(F10.3)'), ' IS NEGLIGIBLE.'
  goto, label_5
...
plot,EXP(-freq_flare), pow_flare,title='Fourier Spectrum - Flare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-4.2,3.]
...
plot,EXP(-freq_post), pow_post,title='Fourier Spectrum - Postflare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], yrange=[-5.7, 2.5], thick=2
...
plot,[0],[0],charsize=ch,/xsty,/ysty,yrange=[-10,20],xrange=[0,300],$   ;ORIGINAL SIGNAL WINDOW
  title='Profile of Flare 375344', xtitle='Elapsed Time [s]',ytitle='Amplitude'
...
oplot,tim_pre,trend_emd_pre -50. ,color=50,thick=4	 ;200 is yelow, 150 And 130 is lime green, 250 red, 50 lbue
