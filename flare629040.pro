...
;CD, '/Users/dmitriikolotkov/Dropbox/Tishtrya/flare629040'
 
A = FindGen(16) * (!PI*2/16.) & UserSym, cos(A), sin(A), /fill
device, decomposed=0 & loadct,39
d=get_screen_size()
flarename= 'flare629040.fits'
...
;for SF =0.001, 0.300, 0.015 DO BEGIN   
;Print, 'Shift-Factor=', SF
set_plot, 'PS'
;device, filename='Flare629040_TREND_' + StrTrim(SF,2)+'.eps', encapsulated=1,$		;DEFINES FILENAME AND IMAGE PROPERTIES
device, filename='629040_1.eps', encapsulated=1,$
...
modes=emd(x_flare-mean(x_flare), shiftfactor=0.095, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
modes=emd(x_post-mean(x_post), shiftfactor=0.061, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
plot,[0],[0],charsize=ch,/xsty,/ysty,yrange=[-20.,23.],xrange=[0,300],$   ;ORIGINAL SIGNAL WINDOW
  title='Profile of Flare 629040', xtitle='Elapsed Time [s]',ytitle='Amplitude'
oplot,tim_flare,trend_emd_flare  -27. ,color=130,thick=4
oplot,tim_flare, x_flare+trend_emd_flare -27.,thick=3
...
oplot,tim_pre, x_pre+trend_emd_pre -27.
oplot,tim_pre,trend_emd_pre -27. ,color=50,thick=4	 ;200 is yelow, 150 is lime green, 250 red, 50 lbue
...
oplot,tim_post, x_post+trend_emd_post -27.,thick=3
oplot,tim_post,trend_emd_post -27. ,color=50,thick=4
