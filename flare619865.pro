...
;CD, '/Users/dmitriikolotkov/Dropbox/Tishtrya/flare619865'
 
A = FindGen(16) * (!PI*2/16.) & UserSym, cos(A), sin(A), /fill
device, decomposed=0 & loadct,39
d=get_screen_size()
flarename= 'flare619865.fits'
...
;for SF =0.01, 0.300, 0.02 DO BEGIN   
;Print, 'Shift-Factor=', SF
set_plot, 'PS'
;device, filename='Flare619865_TREND_POST2_' + StrTrim(SF,2)+'.eps', encapsulated=1,$		;DEFINES FILENAME AND IMAGE PROPERTIES
device, filename='619865_1.eps', encapsulated=1,$
...
modes=emd(x_pre-mean(x_pre), shiftfactor=0.13, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
alpha_2=result[0]
if (alpha_2 lt 0.) then begin
  alpha_2=alpha_1
  break_point = 1d10
  pow_fit_pre=replicate(pow_fit_pre[N_ELEMENTS(pow_fit_pre)-1],N_ELEMENTS(pow_fit_pre))
endif else break_point=exp(-result[1])

print, 'Alpha_est_1 is', alpha_1
print, 'Alpha_est_2 is', alpha_2
if (break_point lt max(exp(-freq_pre)))  then begin
print, 'Breaking point is',break_point,' seconds'
endif else print, 'Breaking point is', ' NaN'
...
;0.03
modes=emd(x_pre-mean(x_pre), shiftfactor=0.15, epsilon=1d-4, maxsiftings=1d5)
...
;SF= 
;around 0.091 ( no less/more than 0.076 or 0.106)
modes=emd(x_flare-mean(x_flare), shiftfactor=0.04, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
if (alpha_2 lt 0.) then begin
  alpha_2=alpha_1
  break_point = 1d10
  pow_fit_flare=replicate(pow_fit_flare[N_ELEMENTS(pow_fit_flare)-1],N_ELEMENTS(pow_fit_flare))
endif else break_point=exp(-result[1])

print, 'Alpha_est_1 is', alpha_1
print, 'Alpha_est_2 is', alpha_2
if (break_point lt max(exp(-freq_flare)))  then begin
print, 'Breaking point is',break_point,' seconds'
endif else print, 'Breaking point is', ' NaN'
...
;0.002
modes=emd(x_flare-mean(x_flare), shiftfactor=0.005, epsilon=1d-4, maxsiftings=1d5)
...
emd_mode_flare=  modes[*,1];+modes[*,2]
...
modes=emd(x_post-mean(x_post), shiftfactor=0.01, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
trend_emd_post=mean(x_post)+modes[*,sz[2]-1] +modes[*, sz[2]-2] ;+modes[*, sz[2]-2]    ;FIND TREND FROM LAST MODE IN EMD SPECTRUM
...
if (alpha_2 lt 0.) then begin
  alpha_2=alpha_1
  break_point = 1d10
  pow_fit_post=replicate(pow_fit_post[N_ELEMENTS(pow_fit_post)-1],N_ELEMENTS(pow_fit_post))
endif else break_point=exp(-result[1])

print, 'Alpha_est_1 is', alpha_1
print, 'Alpha_est_2 is', alpha_2
if (break_point lt max(exp(-freq_post)))  then begin
print, 'Breaking point is',break_point,' seconds'
endif else print, 'Breaking point is', ' NaN'
...
;SF= 0.58 for mode[1]
modes=emd(x_post-mean(x_post), shiftfactor=0.019, epsilon=1d-4, maxsiftings=1d5)
...
plot,EXP(-freq_pre), pow_pre,title='Fourier Spectrum - Preflare', xtitle=$   ;FOURIER POWER SPECTRUM
  ...
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-3.5,2.8]
oplot, EXP(-freq_pre[1:*]), pow_fit_pre, linestyle=2, thick=3
oplot, EXP(-freq_pre[1:*]), pow_fit95_pre, color=150, thick=2
oplot, EXP(-freq_pre[1:*]), pow_fit99_pre, color=250, thick=2
...
plot,EXP(-freq_flare), pow_flare,title='Fourier Spectrum - Flare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-4.5,2.9]
oplot, EXP(-freq_flare[1:*]), pow_fit_flare, linestyle=2, thick=3
oplot, EXP(-freq_flare[1:*]), pow_fit95_flare, color=150, thick=2
oplot, EXP(-freq_flare[1:*]), pow_fit99_flare, color=250, thick=2
...
plot,EXP(-freq_post), pow_post,title='Fourier Spectrum - Postflare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], yrange=[-4.8, 4.4], thick=2
oplot, EXP(-freq_post[1:*]), pow_fit_post, linestyle=2, thick=3
oplot, EXP(-freq_post[1:*]), pow_fit95_post, color=150, thick=2
oplot, EXP(-freq_post[1:*]), pow_fit99_post, color=250, thick=2
...
plot,[0],[0],charsize=ch,/xsty,/ysty,yrange=[-12,23],xrange=[0,300],$   ;ORIGINAL SIGNAL WINDOW
  title='Profile of Flare 619865', xtitle='Elapsed Time [s]',ytitle='Amplitude'
oplot,tim_flare,trend_emd_flare  -25. ,color=130,thick=4
oplot,tim_flare, x_flare+trend_emd_flare -25.,thick=3

oplot,tim_flare,x_flare
oplot,tim_flare,emd_mode_flare,color=250,thick=4

oplot,tim_flare,x_flare-emd_mode_flare -8. 

oplot,tim_pre, x_pre+trend_emd_pre -25.
oplot,tim_pre,trend_emd_pre -25. ,color=50,thick=4	 ;200 is yelow, 150 is lime green, 250 red, 50 lbue

oplot,tim_pre,x_pre
;oplot,tim_pre,emd_mode_pre,color=130,thick=4

oplot,tim_pre,x_pre -8. 

oplot,tim_post, x_post+trend_emd_post -25.,thick=3
oplot,tim_post,trend_emd_post -25. ,color=50,thick=4

oplot,tim_post,x_post
oplot,tim_post,emd_mode_post,color=150,thick=4

oplot,tim_post,x_post -8. ;-emd_mode_post -7. 
