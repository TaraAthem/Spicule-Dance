;----------------------------------------------------------------------------------------
; EMD CODE MODIFIED BY T. MEHTA, JAN 2019
;
; EDITED TO READ IN FERMI DATA
...
datafile='104_fermi_2014-10-26T18:08:00.dat'	
...
tstart 	= '18:12:40 26-OCT-14'
tend 	= '18:15:28 26-OCT-14'
...
!p.charthick = 1
!p.multi = [0,2,2]
utplot, t, x, ystyle = 1, ytitle = 'Fermi Signal'					; LIGHTCURVE OF 'INPUT' DATA AND TREND
...
IF (channel eq 2) THEN print,'This flare was detected by Fermi ( 6-12keV)' ELSE $
IF (channel eq 3) THEN print,'This flare was deteched by Fermi ( 12-25keV)'ELSE $
IF (channel eq 4) THEN print,'This flare was deteched by Fermi ( 25-50keV)'ELSE $
IF (channel eq 5) THEN print,'This flare was deteched by Fermi ( 50-100keV)' ELSE print, 'Something went wrong. Check channel number.'
...
IF (maxemd eq 0) and (LOOP eq 'Y') THEN GOTO, LABEL1					; THIS ENSURES THE PROGRAM CONTINUES RUNNING UNTIL A SIG IMF IS FOUND 
											; FOR A GIVEN SF-THIS MAY NOT NECESSAIRLY BE AN APPROPRIATE ;-----multimodal
mode1=  modes[*,3]
mode2= modes[*,4]
;--------------
trend=trend_emd
