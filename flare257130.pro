...
;CD, '/Users/dmitriikolotkov/Dropbox/Tishtrya/flare257130'
 
A = FindGen(16) * (!PI*2/16.) & UserSym, cos(A), sin(A), /fill
device, decomposed=0 & loadct,39
d=get_screen_size()
flarename= 'flare257130.fits'
...
;for SF =0.01, 0.300, 0.03 DO BEGIN   
;Print, 'Shift-Factor=', SF
set_plot, 'PS'
;device, filename='Flare257130_EMD_' + StrTrim(SF,2)+'.eps', encapsulated=1,$		;DEFINES FILENAME AND IMAGE PROPERTIES
device, filename='257130_1.eps', encapsulated=1,$
...
modes=emd(x_pre-mean(x_pre), shiftfactor=0.04, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
modes=emd(x_pre-mean(x_pre), shiftfactor=SF, epsilon=1d-4, maxsiftings=1d5)
...
;0.09
modes=emd(x_flare-mean(x_flare), shiftfactor=0.0085, epsilon=1d-4, maxsiftings=1d5)
...
emd_mode_flare=  modes[*,2];+modes[*,2]
...
modes=emd(x_post-mean(x_post), shiftfactor=0.04, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
;SF=0.019 almost CI
modes=emd(x_post-mean(x_post), shiftfactor=0.03, epsilon=1d-4, maxsiftings=1d5)
...
plot,EXP(-freq_pre), pow_pre,title='Fourier Spectrum - Preflare', xtitle=$   ;FOURIER POWER SPECTRUM
  ...
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-3.6,2.1]
...
plot,EXP(-freq_flare), pow_flare,title='Fourier Spectrum - Flare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-5.,3.8]
...
plot,EXP(-freq_post), pow_post,title='Fourier Spectrum - Postflare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], yrange=[-5., 3.5], thick=2
...
plot,[0],[0],charsize=ch,/xsty,/ysty,yrange=[-15,25],xrange=[0,300],$   ;ORIGINAL SIGNAL WINDOW
  title='Profile of Flare 257130', xtitle='Elapsed Time [s]',ytitle='Amplitude'
oplot,tim_flare,trend_emd_flare   ,color=130,thick=4
oplot,tim_flare, x_flare+trend_emd_flare ,thick=3

oplot,tim_flare,x_flare
oplot,tim_flare,emd_mode_flare,color=250,thick=4

oplot,tim_flare,x_flare-emd_mode_flare -12. 

oplot,tim_pre, x_pre+trend_emd_pre
oplot,tim_pre,trend_emd_pre  ,color=50,thick=4	 ;200 is yelow, 150 is lime green, 250 red, 50 lbue

oplot,tim_pre,x_pre
;oplot,tim_pre,emd_mode_pre,color=130,thick=4

oplot,tim_pre,x_pre -12. 

oplot,tim_post, x_post+trend_emd_post ,thick=3
oplot,tim_post,trend_emd_post  ,color=50,thick=4

oplot,tim_post,x_post
;oplot,tim_post,emd_mode_post,color=150,thick=4

oplot,tim_post,x_post -12. ;-emd_mode_post -7. 
