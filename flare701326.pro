...
;CD, '/Users/dmitriikolotkov/Dropbox/Tishtrya/flare701326'
 
A = FindGen(16) * (!PI*2/16.) & UserSym, cos(A), sin(A), /fill
device, decomposed=0 & loadct,39
d=get_screen_size()
flarename= 'flare701326.fits'
...
;for SF =0.001, 0.300, 0.03 DO BEGIN   
;Print, 'Shift-Factor=', SF
set_plot, 'PS'
;device, filename='Flare701326_EMD2_' + StrTrim(SF,2)+'.eps', encapsulated=1,$		;DEFINES FILENAME AND IMAGE PROPERTIES
device, filename='701326_1.eps', encapsulated=1,$
...
modes=emd(x_pre-mean(x_pre), shiftfactor=0.049, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
modes=emd(x_flare-mean(x_flare), shiftfactor=0.087, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
modes=emd(x_flare-mean(x_flare), shiftfactor=0.075, epsilon=1d-4, maxsiftings=1d5)
...
emd_mode_flare=modes[*,2];+ modes[*,3]
...
modes=emd(x_post-mean(x_post), shiftfactor=0.069, epsilon=1d-4, maxsiftings=1d5)
sz=size(modes)
...
; IDEALLY IDENTICAL TO THE SYNTHETIC ONE).
;0.151
modes=emd(x_post-mean(x_post), shiftfactor=0.176, epsilon=1d-4, maxsiftings=1d5)
...
plot,EXP(-freq_pre), pow_pre,title='Fourier Spectrum - Preflare', xtitle=$   ;FOURIER POWER SPECTRUM
  ...
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-3.5,2.7]
...
plot,EXP(-freq_flare), pow_flare,title='Fourier Spectrum - Flare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2,yrange=[-4.1,3.]
...
plot,EXP(-freq_post), pow_post,title='Fourier Spectrum - Postflare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], yrange=[-3.5, 2.8], thick=2
...
plot,[0],[0],charsize=ch,/xsty,/ysty,yrange=[-8,18],xrange=[0,300],$   ;ORIGINAL SIGNAL WINDOW
  title='Profile of Flare 701326', xtitle='Elapsed Time [s]',ytitle='Amplitude'
...
oplot,tim_flare,x_flare-emd_mode_flare -5. 
