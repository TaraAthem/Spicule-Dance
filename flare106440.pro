...
set_plot, 'PS'
device, filename='106440_1.eps', encapsulated=1,$		;DEFINES FILENAME AND IMAGE PROPERTIES
...
sz=size(modes)
trend_emd_pre=mean(x_pre)+modes[*,sz[2]-1] +modes[*, sz[2]-2]       ;FIND TREND FROM LAST MODE IN EMD SPECTRUM
...
plot,EXP(-freq_pre), pow_pre,title='Fourier Spectrum - Preflare', xtitle=$   ;FOURIER POWER SPECTRUM
  ...
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2, yrange=[-6.,2.7]
...
plot,EXP(-freq_flare), pow_flare,title='Fourier Spectrum - Flare', xtitle=$   ;FOURIER POWER SPECTRUM
  'Period [s]', ytitle='Log Fourier power [a.u.]', /xstyle, $
  /ystyle, charsize=ch, /xlog, xrange=[2,100], thick=2, yrange=[-4.,3.5]
...
plot,[0],[0],charsize=ch,/xsty,/ysty,yrange=[-10,20],xrange=[0,300],$   ;ORIGINAL SIGNAL WINDOW
  title='Profile of Flare 106440', xtitle='Elapsed Time [s]',ytitle='Amplitude'
...
oplot,tim_pre,x_pre-emd_mode_pre -5. 
